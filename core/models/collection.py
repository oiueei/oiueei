...
class Collection(models.Model):
    ...
    def add_thing(self, thing_code):
        ...
        if thing_code not in self.collection_articles:
            self.collection_articles.append(thing_code)
            self.save(update_fields=["collection_articles"])

    def remove_thing(self, thing_code):
        """Remove a thing from this collection."""
        if thing_code in self.collection_articles:
            self.collection_articles.remove(thing_code)
            self.save(update_fields=["collection_articles"])
